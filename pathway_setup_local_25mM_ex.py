...
f1=open('pathway_ISG_record_25mMglucose+10nMex.txt', 'w')
f2=open('pathway_ISG-NE_distance_25mMglucose+10nMex.xvg', 'w')
f3=open('pathway_ISG-secretion-along-time_25mMglucose+10nMex.xvg','w')
...
N_GRANULES = 791 # Number of granules
...
BD_STEP_SIZE_SEC= 1E-9 # a time step of 0.001 ns
#SIM_TIME_SEC= 0.050 # a total simulation time of 5 ms
SIM_TIME_SEC= 1E-4 # a total simulation time of 100 ns 1ns = 1E-9s
...
print(IMP.core.RigidBody(h_granules_root.get_children()[0]).get_rigid_members()[1:])
print(h_granules_root.get_children()[0])
print(h_granules_root.get_children()[0])
print(IMP.atom.Mass(h_granules_root.get_children()[0]).get_mass_key())
print(IMP.atom.Mass(h_granules_root.get_children()[0]).get_mass())
print(IMP.insulinsecretion.SecretionCounter(h_granules_root.get_children()[0]).get_secretion_key())
print(IMP.insulinsecretion.SecretionCounter(h_granules_root.get_children()[0]).get_secretion())
print(IMP.insulinsecretion.Maturation(h_granules_root.get_children()[0]).get_state())
# Add granule activition
gaos=IMP.insulinsecretion.GranuleActivationOptimizerState(h_granules_root.get_children(), h_glucose_root.get_children(), 1, 0.5, 1) #granule and patches, glucose, contact_range, slack, frame interval
# Add insulin secretion
isos= IMP.insulinsecretion.InsulinSecretionOptimizerState(m, h_granules_root.get_children(), pbc_sphere, nucleus_sphere, 10000, 3000, 1) #granule and patches, glucose, contact_range, slack, frame interval
...
rs = []
gtsc= IMP.insulinsecretion.GranuleTraffickingSingletonScore([0, 0, 0], 1) # Push particles radially away or towards the center of some sphere
...
rmf = RMF.create_rmf_file("pathway_25mMglucose12_47.rmf")
