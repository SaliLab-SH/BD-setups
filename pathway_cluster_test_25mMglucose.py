...
D_GRANULES = 3.2E-6 # A^2/fs, 100,000 times of the real diffusion coeff.
N_GLUCOSE = 800   # 50926 is ok but 200,000 is too large
R_GLUCOSE = 100 # A, 20 times of the real radius
D_GLUCOSE = 9.6E-5 # A^2/fs
R_CLUSTER = R_GRANULES * pow(40/0.74,1.0/3.0)
R_CLUSTER_CUTOFF = R_CLUSTER/3
N_GRANULES_IN_CLUSTER = 40
N_CLUSTER = 2
N_GRANULES = 791 - N_CLUSTER * N_GRANULES_IN_CLUSTER # Number of granules

# record the simulation and ISG distance
f1=open('pathway_ISG_record_25mMglucose_{}.txt'.format(N_CLUSTER), 'w')
f2=open('pathway_ISG-NE_distance_25mMglucose_{}.txt'.format(N_CLUSTER), 'w')
f3=open('pathway_ISG_secretioncounter_25mMglucose_{}.txt'.format(N_CLUSTER), 'w')
...
# K_EXCLUDED_CLUSTER = 0.1 
K_ECSC = 0.1
K_CBSSS = 0.1

# III. Time parameters
#BD_STEP_SIZE_SEC= 10E-8 # a time step of 10 ns
BD_STEP_SIZE_SEC= 1E-9 # a time step of 0.001 ns //time for every frame 
#SIM_TIME_SEC= 0.050 # a total simulation time of 5 ms
SIM_TIME_SEC= 1E-4 # a total simulation time of 100 ns 1ns = 1E-9s //total simulation time
...
cluster_sphere,clugf,cluster = [],[],[]
for i in range(N_CLUSTER):
    cluster_sphere.append(IMP.algebra.Sphere3D(get_random_vector_in_cytoplasm(
                                            IMP.algebra.Sphere3D([0,0,0], R_NUCLEUS+2*(R-R_NUCLEUS)/3),
                                            IMP.algebra.Sphere3D([0,0,0], R_NUCLEUS+R_CLUSTER_CUTOFF+R_CLUSTER)), R_CLUSTER))
    clugf.append(ClusterGranuleFactory.ClusterGranuleFactory(
        model = m,
        default_R = R_GRANULES,
        cluster_sphere = cluster_sphere[i],
        number = N_GRANULES_IN_CLUSTER))
    cluster.append(clugf[i].create_granule_with_interaction_patches("Granule_in_cluster",N_PATCHES))
    for granule in cluster[i]:
        ...
        IMP.display.Colored.setup_particle(granule,IMP.display.get_display_color(5))
...
gaos= IMP.insulinsecretion.GranuleActivationOptimizerState(h_granules_root.get_children(), h_glucose_root.get_children(), 1, 0.5, 1) 
#granule and patches, glucose, contact_range, slack, frame interval
gaos.set_log_level(IMP.TERSE)
#print("Model", gaos.get_model()

# Reset granules
isos = IMP.insulinsecretion.InsulinSecretionOptimizerState(m, h_granules_root.get_children(), pbc_sphere, nucleus_sphere, 10000, 3000, 1)
# particles and patches, cell_sphere, nucleus_sphere, cut_off, contact_range, frame interval
...
rs = []
gtsc= IMP.insulinsecretion.GranuleTraffickingSingletonScore([0, 0, 0],1) 
# Push particles radially away or towards the center of some sphere
rs.append(IMP.container.SingletonsRestraint(gtsc,
                                            h_granules_root.get_children()))
ecsc,cbsss=[],[]
for i in range(N_CLUSTER):
    ecsc.append(IMP.insulinsecretion.EnterClusterSingletonScore(cluster_sphere[i], K_ECSC, R_CLUSTER_CUTOFF))
    rs.append(IMP.container.SingletonsRestraint(ecsc[i],
                                                h_granules_root.get_children()))  
    cbsss.append(IMP.insulinsecretion.ClusterBoundingSphereSingletonScore(cluster_sphere[i], K_CBSSS, R_GRANULES))
    rs.append(IMP.container.SingletonsRestraint(cbsss[i],
                                                h_granules_root.get_children()))  
...
rmf = RMF.create_rmf_file("pathway_{}cluster_{}_{}.rmf".format(N_CLUSTER,K_EXCLUDED,K_CBSSS))
...
while n_frames_left>0:
    ...
    count = []
    for i in range(791):
        count.append(IMP.insulinsecretion.SecretionCounter(h_granules_root.get_children()[i]).get_secretion())
    print(sim_time_frames - n_frames_left, file = f3)
    print('count',sum(count),file = f3)
